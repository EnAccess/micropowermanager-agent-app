...
class PaymentGraphRepository(
    private val service: PaymentGraphService,
    ...
    fun getPaymentGraphData(
        customerId: Long?,
        period: String?,
    ) = service.getPaymentGraph(
        preferences.baseUrl + GET_DASHBOARD_GRAPH_REVENUE_ENDPOINT + (if (customerId != null) "$customerId/graph/" else "graph/") +
            period,
    )
