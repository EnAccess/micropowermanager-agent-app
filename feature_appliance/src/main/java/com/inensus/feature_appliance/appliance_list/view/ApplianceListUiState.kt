...
sealed class ApplianceListUiState {
    data class Loading(
        val type: LoadingApplianceListType,
    ) : ApplianceListUiState()

    data class Success(
        val appliances: List<ApplianceTransaction>,
        val type: LoadingApplianceListType,
    ) : ApplianceListUiState()
    ...
    data class ApplianceTapped(
        val appliance: ApplianceTransaction,
    ) : ApplianceListUiState()
}
